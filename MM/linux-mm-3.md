...
This is the third part of the [chapter](https://linux-insides.gitbook.io/linux-insides/MM/) which describes [memory management](https://en.wikipedia.org/wiki/Memory_management) in the Linux kernel and in the previous [part](https://linux-insides.gitbook.io/linux-insides/MM/linux-mm-2.html) of this chapter we met two memory management related concepts:
...
or in other words somebody wants to access a [page](https://en.wikipedia.org/wiki/Page_%28computer_memory%29), a [page fault](https://en.wikipedia.org/wiki/Page_fault) exception is generated. This is achieved by the fact that the `kmemcheck` marks memory pages as `non-present` (more about this you can read in the special part which is devoted to [Paging](https://linux-insides.gitbook.io/linux-insides/Theory/linux-theory-1.html)). If a `page fault` exception is occurred, the exception handler knows about it and in a case when the `kmemcheck` is enabled it transfers control to it. After the `kmemcheck` will finish its checks, the page will be marked as `present` and the interrupted code will be able to continue execution. There is little subtlety in this chain. When the first instruction of interrupted code will be executed, the `kmemcheck` will mark the page as `non-present` again. In this way next access to memory will be caught again.
...
The second stage will be executed during initialization of the Linux kernel, rather during initialization of early [initcalls](https://linux-insides.gitbook.io/linux-insides/Concepts/linux-cpu-3.html). The second stage is represented by the `kmemcheck_init`:
...
As you may remember from the [second part](https://linux-insides.gitbook.io/linux-insides/Initialization/linux-initialization-2.html) of the Linux kernel initialization chapter, the `page fault` handler is located in the [arch/x86/mm/fault.c](https://github.com/torvalds/linux/blob/16f73eb02d7e1765ccab3d2018e0bd98eb93d973/arch/x86/mm/fault.c) source code file and represented by the `do_page_fault` function. We can see following check from the beginning of the `do_page_fault` function:
...
The `kmemcheck_active` gets `kmemcheck_context` [per-cpu](https://linux-insides.gitbook.io/linux-insides/Concepts/linux-cpu-1.html) structure and return the result of comparison of the `balance` field of this structure with zero:
...
The `kmemcheck` mechanism declares special [tasklet](https://linux-insides.gitbook.io/linux-insides/Interrupts/linux-interrupts-9.html):
...
* [Paging](https://linux-insides.gitbook.io/linux-insides/Theory/linux-theory-1.html)
* [page fault](https://en.wikipedia.org/wiki/Page_fault)
* [initcalls](https://linux-insides.gitbook.io/linux-insides/Concepts/linux-cpu-3.html)
...
* [tasklet](https://linux-insides.gitbook.io/linux-insides/Interrupts/linux-interrupts-9.html)
* [Previous part](https://linux-insides.gitbook.io/linux-insides/MM/linux-mm-2.html)
