...
This is fifth part of the [chapter](https://linux-insides.gitbook.io/linux-insides/Timers/index.html) which describes timers and time management related stuff in the Linux kernel. As you might noted from the title of this part, the `clockevents` framework will be discussed. We already saw one framework in the [second](https://linux-insides.gitbook.io/linux-insides/Timers/linux-timers-2.html) part of this chapter. It was `clocksource` framework. Both of these frameworks represent timekeeping abstractions in the Linux kernel.
...
Where the `get_cpu_mask` returns the cpumask containing just a given `cpu` number. More about `cpumasks` concept you may read in the [CPU masks in the Linux kernel](https://linux-insides.gitbook.io/linux-insides/Concepts/linux-cpu-2.html) part. In the last four lines of code we set callbacks for the clock event device suspend/resume, device shutdown and update of the clock event device state.
...
This is the end of the fifth part of the [chapter](https://linux-insides.gitbook.io/linux-insides/Timers/index.html) that describes timers and timer management related stuff in the Linux kernel. In the previous part got acquainted with the `timers` concept. In this part we continued to learn time management related stuff in the Linux kernel and saw a little about yet another framework - `clockevents`.
...
* [CPU masks in the Linux kernel](https://linux-insides.gitbook.io/linux-insides/Concepts/linux-cpu-2.html)
