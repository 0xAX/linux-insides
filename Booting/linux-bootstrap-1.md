...
As we can read in the [documentation](https://github.com/torvalds/linux/blob/master/Documentation/arch/x86/boot.rst), the bootloader must load the kernel into memory, fill some fields in the kernel setup header and pass control to the kernel code. The kernel image begins with a legacy boot sector, followed by the real-mode setup code. The kernel setup header is a special structure embedded in this early part of the image and provides fields that describe how the kernel should be loaded and started. It begins at offset `0x01F1`, near the end of the first 512-byte sector, and extends into the setup area. We may look at the boot [linker script](https://github.com/torvalds/linux/blob/master/arch/x86/boot/setup.ld) to confirm the value of this offset:
