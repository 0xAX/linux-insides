...
This is the first part of the new chapter of the [linux insides](https://linux-insides.gitbook.io/linux-insides/) book. We have come a long way in the previous [chapter](https://linux-insides.gitbook.io/linux-insides/Initialization/index.html) of this book. We started from the earliest [steps](https://linux-insides.gitbook.io/linux-insides/Initialization/linux-initialization-1.html) of kernel initialization and finished with the [launch](https://linux-insides.gitbook.io/linux-insides/Initialization/linux-initialization-10.html) of the first `init` process. Yes, we saw several initialization steps which are related to the various kernel subsystems. But we did not dig deep into the details of these subsystems. With this chapter, we will try to understand how the various kernel subsystems work and how they are implemented. As you can already understand from the chapter's title, the first subsystem will be [interrupts](http://en.wikipedia.org/wiki/Interrupt).
...
Also, we already know from the previous [part](https://linux-insides.gitbook.io/linux-insides/Booting/linux-bootstrap-3.html) that interrupts can be classified as `maskable` and `non-maskable`. Maskable interrupts are interrupts which can be blocked with the two following instructions for `x86_64` - `sti` and `cli`. We can find them in the Linux kernel source code:
...
* `gs_base` - The `gs` register always points to the bottom of the `fixed_percpu_data`. On the `x86_64`, the `gs` register is shared by per-cpu area and stack canary (more about `per-cpu` variables you can read in the special [part](https://linux-insides.gitbook.io/linux-insides/Concepts/linux-cpu-1.html)).  All per-cpu symbols are zero-based and the `gs` points to the base of the per-cpu area. You already know that [segmented memory model](http://en.wikipedia.org/wiki/Memory_segmentation) is abolished in the long mode, but we can set the base address for the two segment registers - `fs` and `gs` with the [Model specific registers](http://en.wikipedia.org/wiki/Model-specific_register) and these registers can be still be used as address registers. If you remember the first [part](https://linux-insides.gitbook.io/linux-insides/Initialization/linux-initialization-1.html) of the Linux kernel initialization process, you can remember that we have set the `gs` register:
...
* [Previous chapter](https://linux-insides.gitbook.io/linux-insides/Initialization/index.html)
