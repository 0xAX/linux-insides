Process of Linux kernel building
================================================================================

Introduction
--------------------------------------------------------------------------------

I will not tell you how to build and install custom Linux kernel on your machine, because you can find many [resources](https://encrypted.google.com/search?q=building+linux+kernel#q=building+linux+kernel+from+source+code) that will help you make it. Instead, we will know what does occur when you input and execute `make` in the directory with Linux kernel source code in this part. When I just started to learn the source code of Linux kernel, the [Makefile](https://github.com/torvalds/linux/blob/master/Makefile) file was the first file that I opened. And it was scary. :) This [makefile](https://en.wikipedia.org/wiki/Make_%28software%29) contained `1591` lines of code at that time when I wrote this part and it was [third](https://github.com/torvalds/linux/commit/52721d9d3334c1cb1f76219a161084094ec634dc) release candidate.

This makefile is the top makefile in the Linux kernel source code and Linux kernel building starts here. Yes, it is big, but if you've read the source code of Linux kernel you can find that all the directories with source code has its own makefile. Of course it is not real to describe how each source files compile and link. So we will see compilation only for the standard case. Here you will not find building of the kernel documentation, cleaning of the kernel source code, [tags](https://en.wikipedia.org/wiki/Ctags) generation, [cross-compilation](https://en.wikipedia.org/wiki/Cross_compiler) related stuff and etc. We will start from the `make` execution with the standard kernel configuration file and end with the building of the [bzImage](https://en.wikipedia.org/wiki/Vmlinux#bzImage).

It would be good if you're already familiar with the [make](https://en.wikipedia.org/wiki/Make_%28software%29) util, but I will try to describe all the code that should be in this part anyway.
