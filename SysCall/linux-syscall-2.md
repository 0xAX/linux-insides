...
The previous [part](https://linux-insides.gitbook.io/linux-insides/SysCall/linux-syscall-1.html) was the first part of the chapter that describes the [system call](https://en.wikipedia.org/wiki/System_call) concepts in the Linux kernel.
...
You can read more about the `Global Descriptor Table` in the second [part](https://linux-insides.gitbook.io/linux-insides/Booting/linux-bootstrap-2.html) of the chapter that describes the booting process of the Linux kernel.
...
which exchanges the current GS base register value with the value contained in the `MSR_KERNEL_GS_BASE ` model specific register. In other words we moved it on to the kernel stack. After this we point the old stack pointer to the `rsp_scratch` [per-cpu](https://linux-insides.gitbook.io/linux-insides/Concepts/linux-cpu-1.html) variable and setup the stack pointer to point to the top of stack for the current processor:
...
This is the end of the second part about the system calls concept in the Linux kernel. In the previous [part](https://linux-insides.gitbook.io/linux-insides/SysCall/linux-syscall-1.html) we saw theory about this concept from the user application view. In this part we continued to dive into the stuff which is related to the system call concept and saw what the Linux kernel does when a system call occurs.
...
* [previous chapter](https://linux-insides.gitbook.io/linux-insides/SysCall/linux-syscall-1.html)
